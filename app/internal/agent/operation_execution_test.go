...
func (actionTestExecutor) Execute(context.Context, executor.Command) (executor.Result, error) { return executor.Result{}, nil }
...
func (*actionTestDefinition) Discover(context.Context, operation.DiscoverInput) (operation.Discovery, error) { return operation.Discovery{}, errors.New("must not be called") }
func (*actionTestDefinition) Precheck(context.Context, operation.PrecheckInput) (operation.Precheck, error) { return operation.Precheck{}, errors.New("must not be called") }
func (*actionTestDefinition) Plan(context.Context, operation.PlanInput) (operation.Plan, error) { return operation.Plan{}, errors.New("must not be called") }
func (*actionTestDefinition) Impact(context.Context, operation.ImpactInput) (operation.Impact, error) { return operation.Impact{}, errors.New("must not be called") }
func (*actionTestDefinition) Apply(context.Context, operation.ApplyInput) (operation.ApplyResult, error) { return operation.ApplyResult{}, errors.New("bounded test definition does not apply") }
func (*actionTestDefinition) Verify(context.Context, operation.VerifyInput) (operation.Verification, error) { return operation.Verification{Passed: true, Summary: "verified"}, nil }
func (*actionTestDefinition) Rollback(context.Context, operation.RollbackInput) (operation.RollbackResult, error) { return operation.RollbackResult{}, errors.New("bounded test definition does not rollback") }
func (*actionTestDefinition) VerifyRollback(context.Context, operation.VerifyRollbackInput) (operation.Verification, error) { return operation.Verification{Passed: true, Summary: "rollback verified"}, nil }
...
func (provider *actionTestRestore) Create(_ context.Context, request operation.RestorePointRequest) (operation.RestorePoint, error) {
	provider.creates++
	return operation.RestorePoint{ID: "rp-1", ProviderID: provider.ID(), OperationID: request.OperationID, RunID: request.RunID, Targets: request.Targets, Manifest: operation.Artifact{SchemaVersion: "test.restore.v1", Payload: json.RawMessage(`{}`)}}, nil
}
func (*actionTestRestore) Verify(context.Context, operation.RestorePoint) (operation.Verification, error) { return operation.Verification{Passed: true}, nil }
func (*actionTestRestore) Restore(context.Context, operation.RestorePoint, operation.ApplyResult) (operation.RollbackResult, error) { return operation.RollbackResult{}, errors.New("must not be called") }
func (*actionTestRestore) VerifyRestored(context.Context, operation.RestorePoint, operation.RollbackResult) (operation.Verification, error) { return operation.Verification{Passed: true}, nil }
...
func newActionTestRunner(t *testing.T) (*operationExecutionRunner, operation.Metadata, *actionTestRestore) {
	...
	if err := registry.Register(&actionTestDefinition{metadata: metadata}); err != nil { t.Fatal(err) }
	restore := &actionTestRestore{}
	runner, err := NewOperationExecutionRunner(registry, restore, actionTestExecutor{}, "linux")
...
func actionTask(t *testing.T, metadata operation.Metadata, action task.OperationAction) task.Resource {
	...
	if action == task.OperationActionVerifyRollback { contract.Rollback = &operation.RollbackResult{} }
...
func TestOperationExecutionRunnerIsBoundedAndDoesNotOwnCoordinatorLifecycle(t *testing.T) {
	...
	if err != nil || result.Action != task.OperationActionCreateRestorePoint || result.RestorePoint == nil || restore.creates != 1 { t.Fatalf("result=%#v creates=%d err=%v", result, restore.creates, err) }
	encoded, err := json.Marshal(result)
	if err != nil { t.Fatal(err) }
	for _, forbidden := range []string{"lifecycle", "journal", "lease", "ledger", "runtime_snapshot"} {
		if containsJSONKey(encoded, forbidden) { t.Fatalf("Agent result owns forbidden %q state: %s", forbidden, encoded) }
	}
}
...
func TestOperationExecutionRunnerDestructiveActionsRequireBothServerAuthorityAdapters(t *testing.T) {
	...
	for _, action := range []task.OperationAction{task.OperationActionApply, task.OperationActionRollback} {
		...
		if err == nil || result.Error == nil || result.Error.Code != "operation_authority_unavailable" { t.Fatalf("action=%s result=%#v err=%v", action, result, err) }
	}
}
...
func TestOperationExecutionRunnerRejectsSecretRefsAndTargetMismatch(t *testing.T) {
	...
	if err == nil || result.Error == nil || result.Error.Code != "secret_delivery_unavailable" { t.Fatalf("result=%#v err=%v", result, err) }
	...
	if err == nil || result.Error == nil || result.Error.Code != "operation_target_mismatch" { t.Fatalf("result=%#v err=%v", result, err) }
}
